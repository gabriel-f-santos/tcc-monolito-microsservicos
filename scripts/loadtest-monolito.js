...
// --- Config ---
const BASE = __ENV.BASE_URL || 'http://tcc-monolito-alb-1189235770.us-east-1.elb.amazonaws.com';
...
export default function () {
  ...
  // Registrar
  r = http.post(`${BASE}/api/v1/auth/registrar`, JSON.stringify({
  ...
  // Login
  r = http.post(`${BASE}/api/v1/auth/login`, JSON.stringify({
  ...
  // Criar categoria
  r = http.post(`${BASE}/api/v1/categorias`, JSON.stringify({
  ...
  // Criar produto
  r = http.post(`${BASE}/api/v1/produtos`, JSON.stringify({
  ...
  // Saida
  r = http.post(`${BASE}/api/v1/estoque/${itemId}/saida`, JSON.stringify({
    quantidade: 20,
  }), auth);

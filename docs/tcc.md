...
A injecao de dependencia foi implementada sem bibliotecas externas, utilizando um padrao de "Composition Root" manual: cada "Bounded Context" possui um modulo container que e o unico ponto do sistema que conhece as implementacoes concretas dos repositorios. Os casos de uso recebem interfaces via construtor e desconhecem se estao conectados a PostgreSQL, DynamoDB ou repositorios em memoria (usados em testes). Essa abordagem foi preferida ao mecanismo Depends() do FastAPI por nao acoplar a composicao de dependencias ao "framework", mantendo as camadas de dominio e aplicacao como Python puro — requisito fundamental da "Clean Architecture".
